...
randomly assigning a second clinic identifier to every person.
...
struct Events
  ...
  function Events(
    ...
    cs = Clinics(rs)
    cs.dutycounts .= min.(
      ceil.(Int64, (meanvisits .* cs.patientcounts) ./ (meanload .* length(ds))),
      cs.staffcounts
    )
    ...
    probability = meanvisits / length(ds)
